...
import {getCategories, getMedia, getAttributesList, getAttributesValueList} from 'src/configs/apiConfig'
...
const deliverableType = [{name: 'None', value: 0}, {name: 'All', value: 1}, {
  name: 'Included',
  value: 2
}, {name: 'Excluded', value: 3}]

const productType = [{name: 'Simple Product', value: 'simple_product'}, {
...
export const allList = createAsyncThunk('allList', async params => {
  ...
  await Promise.all([getCategories(params), getCountries(), getMedia(params), getAttributesList(params), getAttributesValueList(params)])
    .then(allResponse => {
console.log("Lists Response: ", allResponse)
      list = {
        categories: allResponse[0].data?.data,
        mediaImages: allResponse[2].data?.data,
        attributesList: allResponse[3].data?.data,
        attributesValuesList: allResponse[4].data?.data,
        countries: allResponse[1],
        indicator,
        deliverableType,
        productType
      }
...
export const list = createSlice({
  ...
  extraReducers: builder => {
    builder.addCase(allList.fulfilled, (state, action) => {
      ...
      state.attributesValuesList = action.payload.attributesValuesList
