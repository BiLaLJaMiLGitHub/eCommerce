...
const ENDPOINTS = {
  ...
  LISTS: {
    ...
    ATTRIBUTES_LIST: '/get_attributes',
    ATTRIBUTES_VALUE_LIST: '/get_attribute_values'
  },
  Media: {
    GET_MEDIA: '/get_media',
  },
...
export const getAttributesList = async () => {
  // const brtoken = setHeaderToken(token)
  const response = await client.post(ENDPOINTS.LISTS.ATTRIBUTES_LIST, getBody(seller))

  return response
}

export const getAttributesValueList = async () => {
  // const brtoken = setHeaderToken(token)
  const response = await client.post(ENDPOINTS.LISTS.ATTRIBUTES_VALUE_LIST, getBody(seller))
